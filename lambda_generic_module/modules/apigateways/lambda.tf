...
resource "aws_api_gateway_model" "postmodel1" {
  ...
  schema = file("${path.module}/models/postmodel1.json")
  # schema = file("/home/ismaeel/Documents/lambda_fucntions/modules/apigateways/models/postmodel1.json")
# modules/apigateways/models/postmodel1.json
}
...
resource "aws_api_gateway_integration" "update_integration" {
  ...
  integration_http_method = "POST"
  type                    = "AWS"
  # uri                     = aws_lambda_function.${create_function_name}.invoke_arn
  uri                     = "${var.update_arn}"
}
...
resource "aws_api_gateway_model" "updatemodel1" {
  ...
  schema = file("${path.module}/models/updatemodel1.json")
...
resource "aws_api_gateway_model" "deletemodel1" {
  ...
  schema = file("${path.module}/models/deletemodel1.json")
...
resource "aws_api_gateway_model" "getmodel1" {
  ...
  schema = file("${path.module}/models/getmodel1.json")
...
resource "aws_lambda_permission" "apigw_lambda2" {
  ...
  function_name = "${var.update_function_name2}"
...
resource "aws_lambda_permission" "delete_apigw_lambda_permission" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = "${var.delete_function_name2}"
  principal     = "apigateway.amazonaws.com"
...
resource "aws_lambda_permission" "creat_apigw_lambda_permission" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = "${var.create_function_name2}"
  principal     = "apigateway.amazonaws.com"
...
resource "aws_lambda_permission" "get_apigw_lambda_permission" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = "${var.get_function_name2}"
  principal     = "apigateway.amazonaws.com"
