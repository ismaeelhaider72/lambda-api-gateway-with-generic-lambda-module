...
resource "aws_iam_role_policy" "create_policy" {
  ...
  policy = file("${path.module}/iam/createpolicy.json")
}
...
resource "aws_iam_role_policy" "get_policy" {
  ...
  # Terraform's "jsonencode" function converts a
  # Terraform expression result to valid JSON syntax.
  policy = file("${path.module}/iam/getpolicy.json")
}
...
resource "aws_iam_role_policy" "delete_policy" {
  ...
  # Terraform's "jsonencode" function converts a
  # Terraform expression result to valid JSON syntax.
  policy = file("${path.module}/iam/deletepolicy.json")
}
...
resource "aws_iam_role_policy" "update_policy" {
  ...
  # Terraform's "jsonencode" function converts a
  # Terraform expression result to valid JSON syntax.
  policy = file("${path.module}/iam/updatepolicy.json")
}
